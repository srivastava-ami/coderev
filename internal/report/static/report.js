...
function renderArchitecture() {
  ...
  if (arch && arch.Packages && arch.Packages.length) {
    ...
    document.getElementById('arch-pkg-map').innerHTML =
...
function renderDocs() {
  ...
  if (docs.length === 0) {
    ...
    if (el) el.innerHTML = '<div style="color:var(--muted);font-size:.85rem;padding:20px">No documentation files found.</div>';
    return;
  }
...
function renderArchStatus(arch) {
  ...
  if (!arch) { el.innerHTML = ''; return; }
  if (arch.Source === 'doc') {
    el.innerHTML = `<div style="background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.2);border-radius:6px;padding:8px 14px;font-size:.78rem;color:var(--pass)">
      ✓ Architecture document: <code>${escHtml(arch.DocFile)}</code>
    </div>`;
  } else {
    el.innerHTML = `<div style="background:rgba(234,179,8,.08);border:1px solid rgba(234,179,8,.2);border-radius:6px;padding:8px 14px;font-size:.78rem;color:var(--advisory)">
...
function renderPackageMap(pkgs) {
  ...
  container.innerHTML = html || '<div style="color:var(--muted);font-size:.85rem;padding:20px">No packages found.</div>';
}
...
const GV = {
  ...
  showDetail(id){
    const n=GV.map[id]; if(!n){ document.getElementById('gdetail').innerHTML=''; return; }
    const clr=GV.nodeColor(n);
    if(n.type==='fn'){
      GV.sel=n.parentId;
    ...
    detail.innerHTML=`<div class="dtitle">${escHtml(n.label)}</div>
  ...
  setupUI(){
    ...
    if(legend){
      ...
      legend.innerHTML=layers.map(l=>`<span><b style="background:${GV.nodeColor({layer:l})}"></b>${l}</span>`).join('');
    }
